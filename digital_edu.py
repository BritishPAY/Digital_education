...
df.drop(['id', 'bdate', 'followers_count', 'graduation', 'relation', 'life_main', 'people_main', 'city', 'last_seen', 'occupation_name', 'career_start', 'career_end'], axis=1, inplace=True)
df.info()


print('У девушек больше телефонов чем у мужчин (1-девушки, 2-мужчины)')
phone = round(df.groupby('sex')['has_mobile'].mean(), 2)
print(phone)
phone.plot(kind = 'pie')
plt.show()
print('Гипотеза №1 опровергнута.')

print('Девушки чаще покупали курс чем мужчины (1-девушки, 2-мужчины)')
course = round(df.groupby('sex')['result'].mean(), 2)
print(course)
course.plot(kind = 'pie')
plt.show()
print('Гипотеза №2 подтверждена.')

print('У девушек чаще установленно фото чем у парней.')
photo = round(df.groupby('sex')['has_photo'].mean(), 2)
print(photo)
photo.plot(kind = 'pie')
plt.show()
